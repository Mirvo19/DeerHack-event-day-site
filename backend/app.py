...
def submissions_admin_required(f):
    @wraps(f)
    def decorated_function(*args, **kwargs):
        if 'submissions_admin_email' not in session:
            # Always redirect to login page for HTML routes
            return redirect(url_for('submissions_login_page'))
        return f(*args, **kwargs)
    return decorated_function

def log_audit(action_type, details):
    admin_email = session.get('admin_email') or session.get('submissions_admin_email', 'unknown')
    try:
...
def submissions_page():
    return send_from_directory(app.template_folder, 'submissions.html')

@app.route('/submissions/login')
def submissions_login_page():
    return send_from_directory(app.template_folder, 'submissions-login.html')
...
# --- Submissions Admin Authentication API ---

@app.route('/submissions/api/auth/login', methods=['POST'])
def submissions_login():
    data = request.get_json()
    email = data.get('email')
    password = data.get('password')
    ...
    try:
        response = auth_supabase.auth.sign_in_with_password({"email": email, "password": password})
        session['submissions_admin_email'] = response.user.email
        log_audit('submissions_admin_login', {'email': email})
        return jsonify({'message': 'Login successful'}), 200
    except AuthApiError as e:
        return jsonify({'error': e.message}), 401
    except Exception as e:
...
@app.route('/submissions/api/auth/logout', methods=['POST'])
@submissions_admin_required
def submissions_logout():
    email = session.pop('submissions_admin_email', None)
    log_audit('submissions_admin_logout', {'email': email})
    return jsonify({'message': 'Logout successful'}), 200

@app.route('/submissions/api/auth/status', methods=['GET'])
def submissions_auth_status():
    if 'submissions_admin_email' in session:
        return jsonify({'logged_in': True, 'email': session['submissions_admin_email']})
    return jsonify({'logged_in': False})
...
def manage_submissions():
    if request.method == 'GET':
        # Require submissions admin authentication to view submissions
        if 'submissions_admin_email' not in session:
            return jsonify({'error': 'Authentication required'}), 401
        try:
            # Get all submissions ordered by team name and creation date
            response = supabase.table('project_submissions').select('*').order('team_name').order('created_at', desc=True).execute()
    ...
    if request.method == 'POST':
        try:
            ...
            required_fields = ['team_name', 'project_name', 'project_description', 'github_link']
            ...
            submission = {
                ...
                'github_link': data['github_link'].strip()
            }
            
            response = supabase.table('project_submissions').insert(submission).execute()
            return jsonify({'message': 'Submission successful', 'data': response.data}), 201
        except Exception as e:
...
@app.route('/api/submissions/<int:submission_id>', methods=['GET', 'PUT', 'DELETE'])
def manage_single_submission(submission_id):
    if request.method == 'GET':
        # Require authentication for GET
        if 'submissions_admin_email' not in session:
            return jsonify({'error': 'Authentication required'}), 401
        try:
            response = supabase.table('project_submissions').select('*').eq('id', submission_id).single().execute()
            return jsonify(response.data), 200
        except Exception as e:
    ...
    if request.method == 'PUT':
        ...
        try:
            data = request.get_json()
            response = supabase.table('project_submissions').update(data).eq('id', submission_id).execute()
            log_audit('submission_updated', {'submission_id': submission_id, 'data': data})
            return jsonify({'message': 'Submission updated', 'data': response.data}), 200
        except Exception as e:
            return jsonify({'error': str(e)}), 500
    ...
    if request.method == 'DELETE':
        ...
        if 'submissions_admin_email' not in session:
            return jsonify({'error': 'Authentication required'}), 401
        try:
            response = supabase.table('project_submissions').delete().eq('id', submission_id).execute()
            log_audit('submission_deleted', {'submission_id': submission_id})
            return jsonify({'message': 'Submission deleted successfully'}), 200
        except Exception as e:
            return jsonify({'error': str(e)}), 500

@app.route('/api/submissions/team/<team_name>', methods=['GET'])
def get_submissions_by_team(team_name):
    try:
        response = supabase.table('project_submissions').select('*').eq('team_name', team_name).order('created_at', desc=True).execute()
        return jsonify(response.data), 200
    except Exception as e:
        return jsonify({'error': str(e)}), 500
